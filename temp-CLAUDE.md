...
### Review Mode (复习模式)
基于艾宾浩斯遗忘曲线的智能复习功能：
1. System calculates next review date based on mistake count and forgetting curve
2. `RecommendationsPage` displays due items with priority levels
3. User clicks "开始复习" to enter `ReviewModePane` (independent from mistake book)
4. In review mode:
   - Answer is hidden initially, user must submit answer
   - After submission, system shows correctness, correct answer and explanation
   - User clicks "返回" to go back to recommendations
5. Backend APIs:
   - `GET /quiz/mistakes/recommendations` - Get review recommendations
   - `POST /quiz/submit-answer` - Submit answer (returns `SubmitAnswerResultVO`)
6. Components:
   - `RecommendationsPage.vue` - Review recommendations list
   - `ReviewModePane.vue` - Standalone review interface (not shared with mistake book)
   - `ReviewCard.vue` - Recommendation card component
